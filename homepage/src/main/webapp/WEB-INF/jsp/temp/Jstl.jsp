	...
	<ul>
		<c:choose>
			...
			<c:when test="${step eq '5'}">
				<li>
					<h3>c:choose c:when, c:otherwise Tag : 조건문 중의 하나로 java에서 사용하는 if, else if, else와 비슷함</h3>
					<h4>기본문법 : &#60;c:choose&#62;<br>
						&#60;c:when test="조건문"&#62; 조건 true에 대한 내용 &#60;/c:when&#62;<br>
						&#60;c:otherwise&#62; 조건에 해당되지 않아서 나오는 내용 &#60;/c:otherwise#62;<br>
						&#60;c:choose&#62;
					</h4>
			...
			<c:when test="${step eq '6'}">
				<li>
					...
					<h4>기본문법 : </br>
						&#60;c:forEach var="변수명" items="반복문변수"&#62;<br/>
						반복문 내용<br/>
						&#60;/c:forEach&#62;
					</h4>
			...
			<c:when test="${step eq '8'}">
				<li>
					...
					URL 1 : <c:url var="link2" value="/temp/jstlImport.do">
								<c:param name="test" value="테스트호출"/>
							</c:url>
							<br/>
					<!-- target blank >> 새 창에서 열기 -->
					<a href="${link1}" target="_blank" title="Tap키를 누르면 html태그 리더기가 읽어준다요">링크 1번</a><br/>
					<!-- title: 장애인의 접근성 편의를 위한 것 -->
			...
			<c:when test="${step eq '10'}">
				<li>
					<h3>fn:indexOf(): 주어진 문자열이 특정 문자열로 끝나는지 확인하는데 사용, 숫자를 반환</h3>
					<%-- 글자 1이 아니라 0부터 시작한다 --%>
			...
			<c:when test="${step eq '11'}">
				<li>
					<h3>fn:startsWith(): 주어진 문자열이 특정 문자열값으로 시작되는지 확인하는 데 사용, 주로 도메인 체크하는 데에 사용하기 좋다</h3>
			...
			<c:when test="${step eq '12'}">
				<li>
					<h3>fn:endsWith(): 주어진 문자열이 특정 문자열값으로 끝나는지 확인하는 데 사용, 주로 확장자 찾을 때 사용하기 좋다</h3>
					<c:set var="String" value="Welcome to JSP programming"/>
			...
			<c:when test="${step eq '14'}">
				<li>
					...
					<%-- ^ This is first string의 length를 반환함 --%>
					Length of the String-2 is <c:out value="${fn:length(strSplit)}"/>
				</li>
			</c:when>
			<c:when test="${step eq '15'}">
				<li>
					<h3>fn:toLowerCaser(): 문자열의 모든 문자를 소문자로 변환, JSP보다는 JAVA에서 더 사용할 거야</h3>
					<c:set var="string" value="IT IS DIFFICULT FOR ME TO PROGRAM JAVA"/>
					<c:out value="${fn:toLowerCase(string)}"/>
				</li>
			</c:when>
			<c:when test="${step eq '16'}">
				<li>
					<h3>fn:toUpperCaser(): 문자열의 모든 문자를 대문자로 변환</h3>
					<c:set var="string" value="it is difficult for me to program java"/>
			...
			<c:when test="${step eq '19'}">
				<li>
					<h3>fn:trim(): 문자열의 양쪽 긑에서 공백을 제거</h3>
					주로 자바에서 사용한다
					...
					html상에서는 여러번 띄어쓰기 한 것이 한 번만 띄어쓰기 한 것처럼 출력해서 보여준다
				</li>
			</c:when>
			<%-- formatting tag 설명 --%>
			<c:when test="${step eq '20'}">
				<li>
					<h3>fn:formatDate: 제공된 패턴 및 스타일을 사용하여 시간 및 날짜를 형식화함</h3>
					<c:set var="Date" value="<%=new java.util.Date()%>"/>
					${Date}<br/>
					<fmt:formatDate value="${Date}" pattern="yyyy/MM/dd"/>
											<%-- 패턴의 대소문자 원하는 포맷에 따라서 잘 선택해서 써야 --%>
				</li>
			</c:when>
		</c:choose>
	</ul>
