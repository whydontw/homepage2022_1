		...
		<div id="contents">
				...
				<form name="frm" method="post" action="/admin/rsv/rsvSelectList.do">
					<fieldset>
						...
						<select name="searchCondition" id="ftext">
							<option value="0"
								<c:if test="${searchVO.searchCondition eq '0'}">selected="selected"</c:if>>프로그램명</option>
							<option value="1"
								<c:if test="${searchVO.searchCondition eq '1'}">selected="selected"</c:if>>내용</option>
						</select>
			...
			<div id="bbs_wrap">
					...
					<table class="list_table">
						<thead>
							<tr>
								...
								<th class="col">신청기간</th>
								<th class="col">운영일</th>
								<th class="col">운영시간</th>
								<th class="col">강사명</th>
								<th class="col">신청자</th>
								<th class="col">관리</th>
							</tr>
						...
						<tbody>
							<c:forEach var="result" items="${resultList }" varStatus="status">
								<tr>
									...
									<td>
										<c:out value="${result.useBeginTime }~${result.useEndTime }"/>
									</td>
									<td>
				...
				<div id="paging">
					...
					<c:set var="pagingParam"><c:out value="${pageUrl }"/></c:set> <!-- 여기 $ 맞는지 확인해봐야것서 -->
