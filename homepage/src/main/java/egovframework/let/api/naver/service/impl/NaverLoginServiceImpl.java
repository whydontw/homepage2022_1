...
public class NaverLoginServiceImpl extends EgovAbstractServiceImpl implements NaverLoginService {
	...
	public String getAuthorizationUrl(HttpSession session, String domain) {
		...
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET).callback(redirectUri).state(state).build(NaverLoginApi.instance());
	...
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state, String domain) throws IOException{
		...
		if(StringUtils.pathEquals(sessionState, state)) { //org.springframework.util.StringUtils
			OAuth20Service oauthService = new ServiceBuilder()
					.apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET)
					.callback(redirectUri).state(state)
					.build(NaverLoginApi.instance());
		
		//scribe에서 제공하는 AccessToken 획득 기능으로 네어라 Access Token을 획득
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException{
		...
		Request request = new Request.Builder() //okhttp3.Request
				.url(PROFILE_API_URL).method("GET", null).addHeader("Content-Type", "application/x-www-form-urlencoded").addHeader("Authorization", "Bearer " + oauthToken.getAccessToken()).build();
